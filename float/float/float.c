...
static void float_thd(void *arg) {
	...
	while (!VESC_IF->should_terminate()) {
		...
		case (RUNNING_UPSIDEDOWN):
			...
			float surge_ramp = 0.03; //How long until reaching 90% maximum surge duty, in seconds. 0 < surge_ramp <= surge_cycle
			float surge_anglemin = 1; // Minimum d->proportional required to ensure we are continuously at an acceleration angle
			float duty_increment = (float)0.002716 * pow(surge_ramp, -1.009702); //Formula to calc increment based on time to 90% target value at 832hz
